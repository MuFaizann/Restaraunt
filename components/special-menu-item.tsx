    ...
    <div className={cn("flex items-start gap-4 py-4 border-b border-gray-600 last:border-b-0", className)}>
      ...
      <div className="flex-grow">
        <div className="flex justify-between items-start mb-1">
          <h3 className="text-2xl font-bold text-white font-bebas-neue">{name}</h3>
          {price && <span className="text-lg font-bold text-orange-400 ml-2 flex-shrink-0 font-lora">{price}</span>}
        </div>
        <p className="text-gray-200 text-med whitespace-pre-line font-work-sans">{description}</p>
      </div>
    </div>
  )
}
